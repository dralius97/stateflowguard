import { Guard, GuardMode, StatelessSchema, TransitionResult } from "../../types";
import { Helpers } from "./helpers";

export class TransitionEvaluator<T extends StatelessSchema> {
  private common: Helpers

  constructor(public transition: T, private guardMap: Map<string, Map<string, Set<string>>> | undefined) {
  ...
  public evaluateRequest(currState: keyof T, event: string, stateGuard?: Record<string, unknown>, eventGuard?: Record<string, unknown>): TransitionResult {
  ...
  public hashBasedEvaluateRequest(currState: keyof T, event: string, stateGuard?: Record<string, unknown>, eventGuard?: Record<string, unknown>): TransitionResult {
    const curr = this.transition[currState]
    if (curr === 'FINAL') return { isValid: false, message: 'Current state is FINAL', reason: `event '${event}' is FINAL state cannot be changed`, code: 'FINAL_STATE' }
    if (!curr) return { isValid: false, message: 'state invalid', reason: `state '${String(currState)}' not found`, code: 'VALIDATION_ERROR' }
